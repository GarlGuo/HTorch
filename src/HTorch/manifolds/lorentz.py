...
class Lorentz(Manifold):
    ...
    def check(self, x:Tensor, c:Union[float,Tensor]) -> Tensor:
        return torch.all(
            torch.isclose(self.inner(x,x), torch.tensor(abs(1./c), dtype=x.dtype)))
    
    def check_tan(self, x, x_tan, c):
        res = self.inner(x, x_tan, c=c)
        return torch.all(torch.isclose(res, torch.zeros_like(res)))
        
    def origin(self, d, c, size:Size=None, device:device=None, dtype:dtype=None):
        ret = torch.zeros(*size, d + 1, device=device, dtype=dtype)
        ret[..., -1] = math.sqrt(abs(1 / c))
        return ret
