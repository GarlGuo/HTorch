...
def _Grow_ExpN(x_tensor, value, simple=True):
    nc = x_tensor.size(-1)
    Q = value
    h = torch.zeros((x_tensor[..., 0] + value).shape +
                    (nc,), dtype=x_tensor.dtype, device=x_tensor.device)
...
def _AddMCN(x_tensor, y_tensor, simple=True):
    nc = x_tensor.size(-1)
    h = torch.zeros((x_tensor[..., 0] + y_tensor[..., 0]).shape + (nc, ), 
...
def _sqrt(input_tensor):
    ...
    x = torch.zeros_like(input_tensor)
    x[..., 0].data.copy_(1 / torch.sqrt(input_tensor[..., 0]))  # warm start
    x = torch.nan_to_num(x, posinf=0, neginf=0)
    # print(x.shape) # 32 51 1 2
    # print((input_tensor[..., 0] == 0).shape)
    # x[input_tensor[..., 0] == 0, 0] = 0
    for _ in range(2):
        x_sq = _MultMCN(x, x)
        x = x + x * (1 - _MultMCN(input_tensor, x_sq)) / 2
...
def _norm(x: torch.Tensor, dim=None, keepdim=False, p=2):
    ...
    if p == float('inf'):
        ...
        if keepdim:
            ...
            return res
    elif p == 2 or p == 'fro': # special L2 norm
...
def _arsinh(x):
    res = _AddMCN(x, _sqrt(_AddMCN(_square(x), num_to_mctensor_like(1, x))))
    res = _clamp(res, min=1e-15)
    res = _log(res)
    return res

def _arcosh(x):
    x = _clamp(x, min=1.0 + 1e-15)
    res = _AddMCN(x, _sqrt(_AddMCN(_square(x), num_to_mctensor_like(-1, x))))
    res = _clamp(res, 1e-15)
    res = _log(res)
    return res
