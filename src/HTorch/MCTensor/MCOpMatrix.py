...
import collections
import torch.sparse
from HTorch.MCTensor.MCOpBasics import _AddMCN, _ScalingN, _MultMCN
...
def _Dot_MCN_M(x_tensor, y_tensor):
    """
    Dot product between tensor of a MCTensor(x_tensor) and a MCTensor(y_tensor),
    produce a sclar
    """
    tmp = _MultMCN(x_tensor, y_tensor)
    ret = tmp[0]
...
def _MV_MC_M_M(x_tensor, y_tensor):
    scaled = _MultMCN(x_tensor, y_tensor)
    tmp = scaled[..., 0, :]
    for i in range(1, scaled.size(-2)):
        tmp = _AddMCN(tmp, scaled[..., i, :])
    return tmp
...
def _MM_MC_MC(x_tensor, y_tensor):
    scaled = _MultMCN(x_tensor.unsqueeze(-3), y_tensor.transpose(-2, -3))
    tmp = scaled[..., 0, :]
    for i in range(1, scaled.size(-2)):
        tmp = _AddMCN(tmp, scaled[..., i, :])
    return tmp
...
def _BMM_MC_MC(x_tensor, y_tensor):
    x1, x2, _, nc = x_tensor.size()
    y1, _, y3, nc = y_tensor.size()
    size = max(x1, y1), x2, y3
    scaled = _MultMCN(x_tensor.unsqueeze(-3), y_tensor.unsqueeze(1).transpose(-2, -3))
    tmp = scaled[..., 0, :]
    for i in range(1, scaled.size(-2)):
        tmp = _AddMCN(tmp, scaled[..., i, :])
    return tmp, size, nc
